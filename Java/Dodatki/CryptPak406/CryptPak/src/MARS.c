/*
 *
 * MARS - A 128bit blocksize 256bit keysize cipher
 * implementing the MARS algorithm in CBC mode
 *
 * Written by Walter Dvorak <e9226745@student.tuwien.ac.at>
 *
 * For details in the MARS Encryption Algorithm refer:
 * [1] "MARS - a candidate cipher for AES",
 *      IBM Corporation, 17 July 1998
 *
 * This work based in parts on a opt. MARS implementation
 *   from Dr Brian Gladman <gladman@seven77.demon.co.uk>
 *
 * converted to a CryptPak cipher by Markus Hahn <markus_hahn@gmx.net>
 * (on 14 April 2000), reformatted on 08/04/00
 *
 * The algorithm MARS is covered by a pending patent
 * application owned by IBM.
 *
 * This program is public domain. Non-profit use only!
 *
 * Date: 3.10.1998
 *
 */

#define MARS_KEYSIZE    32              /* 256 Bit Key */
#define MARS_BLOCKSIZE  16              /* 128 Blocksize */

#include "MARS.h"

static WORD32 s_box[0x200] = 
{
...
static WORD32 vk[47] = {
    0x09d0c479, 0x28c8ffe0, 0x84aa6c39, 0x9dad7287,
    0x7dff9be3, 0xd4268361, 0xc96da1d4, 0x00000000,
...
typedef struct 
{
   WORD32 l_key[40];                     /* Key after expansion */
   WORD32 CBC[4];                        /* CBC IV */
} 
MARSCTX;
...
/*
 * Circular right and left shift
 */
#define rotr(x,n)   (((x) >> ((WORD16)(n))) | ((x) << (32 - (WORD16)(n))))
#define rotl(x,n)   (((x) << ((WORD16)(n))) | ((x) >> (32 - (WORD16)(n))))

/*
 * MARS uses four mix (start/end) and four kt (middle) round functions
 */
...
#define f_kt2(a,b,c,d,i)              \
    m = a + (key->l_key)[i + i + 20]; \
    a = rotl(a, 13);                  \
    r = a * (key->l_key)[i + i + 21]; \
    l = s_box[m & 0x000001ff];        \
    r = rotl(r, 5); l ^= r;           \
    c += rotl(m, r & 0x0000001f);     \
    r = rotl(r, 5); l ^= r;           \
...
/*
 * gen_mask is used in the key generation function
 */
#define gen_mask(x, m)                          \
{   WORD32  u;                                  \
    m = x & (x >> 1); m &= (m >> 2);            \
...
/*
 * Key schedual
 * Note: key_len is the length in doubles of WORD32, not WORD8.
 *       For example: 256bit Key == 32 key-bytes, so key_len=8)
 */
void _mars_setkey
  (MARSCTX* key, 
   WORD32* key_blk, 
   WORD32 key_len)
{
    WORD32  i, j, m, u, w, *t = vk + 7;

    for (i=0; i<39; ++i) 
    {
       u = t[i - 7] ^ t[i - 2];
       t[i] = rotl(u, 3) ^ key_blk[i % key_len] ^ i;
    }
    ...
    for (j=0; j<7; ++j) 
    {
       for(i=1; i<40; ++i) 
       {
            u = t[i] + s_box[t[i - 1] & 0x000001ff];
            t[i] = rotl(u, 9);
       }
       u = t[0] + s_box[t[39] & 0x000001ff];
       t[0] = rotl(u, 9);
    }
    for(i=0; i<40; ++i) 
    {
        (key->l_key)[(7 * i) % 40] = t[i];
    }
    for(i=5; i<37; i+=2) 
    {
        u = s_box[265 + ((key->l_key)[i] & 0x00000003)];
        j = (key->l_key)[i + 3] & 0x0000001f;
        w = (key->l_key)[i] | 0x00000003;
        gen_mask(w, m);
        (key->l_key)[i] = w ^ (rotl(u, j) & m);
    }
    /* Wipe clean */
    for (i=0; i<40; i++) 
    {
...
void _mars_encrypt
  (MARSCTX* key, 
   ...
   WORD32* out_blk)
{
    register WORD32  a, b, c, d, l, m, r;

    /* Get inblock into a,b,c,d */
    a = in_blk[0] + (key->l_key)[0];
    b = in_blk[1] + (key->l_key)[1];
    c = in_blk[2] + (key->l_key)[2];
    d = in_blk[3] + (key->l_key)[3];

    /* unrolled encryption loops */
    f_mix(a,b,c,d); a += d;
    f_mix(b,c,d,a); b += c;
    f_mix(c,d,a,b);
    f_mix(d,a,b,c);
    ...
    f_kt1(a,b,c,d,0); f_kt1(b,c,d,a,1);
    f_kt1(c,d,a,b,2); f_kt1(d,a,b,c,3);
    f_kt1(a,b,c,d,4); f_kt1(b,c,d,a,5);
    f_kt1(c,d,a,b,6); f_kt1(d,a,b,c,7);

    f_kt2(a,b,c,d,0); f_kt2(b,c,d,a,1);
    f_kt2(c,d,a,b,2); f_kt2(d,a,b,c,3);
    f_kt2(a,b,c,d,4); f_kt2(b,c,d,a,5);
    f_kt2(c,d,a,b,6); f_kt2(d,a,b,c,7);
    ...
    out_blk[0] = a - (key->l_key)[36];
    out_blk[1] = b - (key->l_key)[37];
    out_blk[2] = c - (key->l_key)[38];
    out_blk[3] = d - (key->l_key)[39];
}
   ...
   WORD32* out_blk)
{
    ...
    /* Get inblock into a,b,c,d */
    a = in_blk[0] + (key->l_key)[36];
    b = in_blk[1] + (key->l_key)[37];
    c = in_blk[2] + (key->l_key)[38];
    d = in_blk[3] + (key->l_key)[39];

    /* unrolled decryption loops */
    rb_mix(d,a,b,c); d += a;
    rb_mix(c,d,a,b); c += b;
    rb_mix(b,c,d,a);
    rb_mix(a,b,c,d);
    ...
    rf_kt2(d,a,b,c,7); rf_kt2(c,d,a,b,6);
    rf_kt2(b,c,d,a,5); rf_kt2(a,b,c,d,4);
    rf_kt2(d,a,b,c,3); rf_kt2(c,d,a,b,2);
    rf_kt2(b,c,d,a,1); rf_kt2(a,b,c,d,0);

    rf_kt1(d,a,b,c,7); rf_kt1(c,d,a,b,6);
    rf_kt1(b,c,d,a,5); rf_kt1(a,b,c,d,4);
    rf_kt1(d,a,b,c,3); rf_kt1(c,d,a,b,2);
    rf_kt1(b,c,d,a,1); rf_kt1(a,b,c,d,0);
    ...
    /* Put a,b,c,d into outblock */
    out_blk[0] = a - (key->l_key)[0];
    out_blk[1] = b - (key->l_key)[1];
    out_blk[2] = c - (key->l_key)[2];
    out_blk[3] = d - (key->l_key)[3];
  ...
  (CIPHERINFOBLOCK* pInfo) 
{
  ...
  tempinfo.lBlockSize = MARS_BLOCKSIZE;
  tempinfo.lKeySize = MARS_KEYSIZE; 
  tempinfo.blOwnHasher = BOOL_FALSE;
  tempinfo.lInitDataSize = MARS_BLOCKSIZE;
  tempinfo.lContextSize = sizeof(MARSCTX);
 ...
 * Driver Selftest
 */
WORD32 Mars_SelfTest 
  (void* pTestContext) 
{

  /* test the driver for correct encrypting and decrypting... */
  MARSCTX* testCtx = (MARSCTX*) pTestContext;
  WORD32 i, tv_t[4];

  /*
   * Using the original testvectors published by IBM-Research
   * Only KEYSIZE=256 testvalues
   */
  static WORD32 tval[12][8] = 
  {
       {0x9867a1fb, 0x22ef7a3e, 0x8ce27c31, 0xa3e1aa02,  /* 1. key 256bit */
        0x3ccce5e8, 0x2aa8beed, 0x9ac3db99, 0x27725ed6},
       {0xa4ab4413, 0x0847c4d3, 0x1621a7a8, 0x8493f4d4,  /* 1. plaintext */
        0x67a1acdd, 0xbe3163e3, 0x5f9f1c2c, 0xb8a48fe3}, /* 1. ciphertext */

       {0xffc60d26, 0x9cde19dd, 0xd37d601d, 0x1b4525e1,  /* 2. */
        0x9867a1fb, 0x22ef7a3e, 0x8ce27c31, 0xa3e1aa02},
       {0x67a1acdd, 0xbe3163e3, 0x5f9f1c2c, 0xb8a48fe3,
        0xc63570e9, 0xde1e37c8, 0xc48f2865, 0xe6afaf2c},

       {0x39f37dcf, 0x42c02e15, 0x17f24878, 0xfdea8acd,  /* 3. */
        0xffc60d26, 0x9cde19dd, 0xd37d601d, 0x1b4525e1},
       {0xc63570e9, 0xde1e37c8, 0xc48f2865, 0xe6afaf2c,
        0x10384856, 0xbea4a8e8, 0x953df99b, 0x57183fe5},

       {0x29cb3599, 0xfc6486fd, 0x82cfb1e3, 0xaaf2b528,  /* 4. */
        0x39f37dcf, 0x42c02e15, 0x17f24878, 0xfdea8acd},
       {0x10384856, 0xbea4a8e8, 0x953df99b, 0x57183fe5,
        0xd7947a79, 0x661e3c2c, 0x77e54798, 0xc9fb6316},

       {0xfe5f4fe0, 0x9a7abad1, 0xf52af67b, 0x6309d63e,  /* 5. */
        0x29cb3599, 0xfc6486fd, 0x82cfb1e3, 0xaaf2b528},
       {0xd7947a79, 0x661e3c2c, 0x77e54798, 0xc9fb6316,
        0x507c07ea, 0x17a6c354, 0x51809575, 0x3be21308},

       {0xae23480a, 0x8ddc7985, 0xa4aa630e, 0x58ebc536,  /* 6. */
        0xfe5f4fe0, 0x9a7abad1, 0xf52af67b, 0x6309d63e},
       {0x507c07ea, 0x17a6c354, 0x51809575, 0x3be21308,
        0xb6766c13, 0x0c169845, 0xbc5d4d82, 0xa310c9c9}
  };


  for (i=0; i<12; i+=2) {

      _mars_setkey (testCtx, tval[i], 8);

      _mars_encrypt(testCtx, tval[i+1], tv_t);

     if ((tval[i+1][4] != tv_t[0]) || (tval[i+1][5] != tv_t[1])
      || (tval[i+1][6] != tv_t[2]) || (tval[i+1][7] != tv_t[3]))
         return CIPHER_ERROR_INVALID;

     _mars_decrypt(testCtx, tv_t, tv_t);

     if ((tval[i+1][0] != tv_t[0]) || (tval[i+1][1] != tv_t[1])
      || (tval[i+1][2] != tv_t[2]) || (tval[i+1][3] != tv_t[3]))
         return CIPHER_ERROR_INVALID;
  }
...
{
  WORD32* pCBCIV;    
  MARSCTX* pCtx = (MARSCTX*) pContext;

  /* do the key setup. Note: The keysize is in WORD32 and not WORD8 */
  _mars_setkey(pCtx, (WORD32*) pKey, (WORD32) (lKeyLen >> 2) );

  /* for encryption create a CBC IV */
  pCBCIV = (WORD32*) pInitData;
  if (lMode == CIPHER_MODE_ENCRYPT)
  GetRndBytes((WORD8*) pCBCIV, MARS_BLOCKSIZE, pRndGenData);
  /* set the CBC IV */
  copy4(pCtx->CBC,pCBCIV);

  /* In MARS are no weak keys known */
  return CIPHER_ERROR_NOERROR;
}
 ...
 */
WORD32 Mars_DestroyWorkContext
  (void* pContext) 
{
  /* clear the context */
  int nI;
  WORD8* clearIt = (WORD8*) pContext;
  for (nI = 0; nI < sizeof(MARSCTX); nI++) clearIt[nI] = 0x00;
  return CIPHER_ERROR_NOERROR;
}
   ...
   WORD32 lNumOfBytes) 
{
  ...
  /* anything to encrypt? */
  if (lNumOfInts < 4) return;

  for (lI = 0; lI < lNumOfInts; lI += 4) 
  {
    /* copy and chain the first block */
    pOutBuf[lI]     = pInBuf[lI]     ^ pCtx->CBC[0];
    pOutBuf[lI + 1] = pInBuf[lI + 1] ^ pCtx->CBC[1];
    pOutBuf[lI + 2] = pInBuf[lI + 2] ^ pCtx->CBC[2];
    pOutBuf[lI + 3] = pInBuf[lI + 3] ^ pCtx->CBC[3];

    /* encrypt one block */
    _mars_encrypt(pCtx, &pOutBuf[lI], &pOutBuf[lI]);

    /* set the new CBC iv */
    copy4(pCtx->CBC,&pOutBuf[lI]);
  }
}
   ...
   const void* pPreviousBlock) 
{
  WORD32 lNumOfInts;
  WORD32 lI;
  WORD32 SaveCBC[4];                  /* backup for the CBC value */
  WORD32* pInBuf = (WORD32*) pSource;
  WORD32* pOutBuf = (WORD32*) pTarget;
  ...
  {
    copy4(pCtx->CBC,pPrevBlock);
  }

  /* work through all blocks... */
  for (lI = 0; lI < lNumOfInts; lI += 4) 
  {
     /* Save inbuf. A MUST if inbuf==outbuf  */
     copy4(SaveCBC,&pInBuf[lI]);

     /* decrypt the block */
     _mars_decrypt(pCtx, &pInBuf[lI], &pOutBuf[lI]);

     /* dechain the recent block */
     pOutBuf[lI]     ^= pCtx->CBC[0];
     pOutBuf[lI + 1] ^= pCtx->CBC[1];
     pOutBuf[lI + 2] ^= pCtx->CBC[2];
     pOutBuf[lI + 3] ^= pCtx->CBC[3];

     /* set the new CBC iv */
     copy4(pCtx->CBC,SaveCBC);
  }
}
